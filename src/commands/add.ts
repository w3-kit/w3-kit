...
async function listAvailableRecipes(): Promise<string[]> {
  const entries = await fetchGithubDirectory("w3-kit/learn", "recipes");
...
async function addRecipe(
  ...
  try {
    allFiles = await fetchDirectoryRecursive("w3-kit/learn", `recipes/${name}`);
  } catch {
...
async function addContract(name: string | undefined): Promise<void> {
  if (!name) {
    ...
    try {
      const entries = await fetchGithubDirectory("w3-kit/contracts", "contracts");
  ...
  try {
    files = await fetchDirectoryRecursive("w3-kit/contracts", `contracts/${name}`);
  } catch {
